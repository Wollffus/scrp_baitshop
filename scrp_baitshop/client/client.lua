...
AddEventHandler('scrp_baitshop:hasEnteredMarker', function(zone)
    currentZone     = zone
end)

AddEventHandler('scrp_baitshop:hasExitedMarker', function(zone)
    if active == true then
        PromptSetEnabled(ShopPrompt, false)
        PromptSetVisible(ShopPrompt, false)
        active = false
...
Citizen.CreateThread(function()
    ...
    while true do
		...
		if isInMarker and not hasAlreadyEnteredMarker then
			...
			TriggerEvent('scrp_baitshop:hasEnteredMarker', currentZone)
		end

		if not isInMarker and hasAlreadyEnteredMarker then
			hasAlreadyEnteredMarker = false
			TriggerEvent('scrp_baitshop:hasExitedMarker', lastZone)
		end
    ...
    while true do
        ...
        elseif WarMenu.IsMenuOpened('buy') then
            if WarMenu.Button('Buy Fishing Rod for ~pa~10$') then
				TriggerServerEvent("scrp_baitshop:buyrod", 10, "WEAPON_FISHINGROD", 1)  
            elseif WarMenu.Button('Buy Bread Bait for ~pa~2$') then
				TriggerServerEvent("scrp_baitshop:buybait", 2, "p_baitBread01x", 1)
			elseif WarMenu.Button('Buy Cheese Bait for ~pa~2$') then
				TriggerServerEvent("scrp_baitshop:buybait", 2, "p_baitCheese01x", 1)
			elseif WarMenu.Button('Buy Corn Bait for ~pa~2$') then
				TriggerServerEvent("scrp_baitshop:buycarrot", 2, "wildcarrot", 1)
			elseif WarMenu.Button('Buy Corn Bait for ~pa~2$') then
				TriggerServerEvent("scrp_baitshop:buybait", 2, "p_baitCorn01x", 1)
			elseif WarMenu.Button('Buy Cricket Bait for ~pa~2$') then
				TriggerServerEvent("scrp_baitshop:buybait", 2, "p_baitCricket01x", 1)
			elseif WarMenu.Button('Buy River Bait for ~pa~2$') then
				TriggerServerEvent("scrp_baitshop:buybait", 2, "p_finishedragonfly01x", 1)
			elseif WarMenu.Button('Buy Lake Bait for ~pa~2$') then
				TriggerServerEvent("scrp_baitshop:buybait", 2, "p_FinisdFishlure01x", 1)
			elseif WarMenu.Button('Buy Swamp Bait for ~pa~2$') then
				TriggerServerEvent("scrp_baitshop:buybait", 2, "p_finishdcrawd01x", 1)
            end
            WarMenu.Display()
			
        elseif WarMenu.IsMenuOpened('sell') then
            if WarMenu.Button('Sell your small fish') then
				TriggerServerEvent("scrp_baitshop:sellsmallfish", 1)  
            elseif WarMenu.Button('Sell your medium fish') then
				TriggerServerEvent("scrp_baitshop:sellmediumfish", 1)
			elseif WarMenu.Button('Sell your large fish') then
				TriggerServerEvent("scrp_baitshop:selllargefish", 1)
            end
            WarMenu.Display()
...
RegisterNetEvent('scrp_baitshop:alert')	
AddEventHandler('scrp_baitshop:alert', function(txt)
